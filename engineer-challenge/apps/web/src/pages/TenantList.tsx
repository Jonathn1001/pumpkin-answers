...
export function TenantList() {
  ...
  function confirmDelete(t: Tenant) {
    modal.confirm({
      title: `Delete ${t.name}?`,
      content:
        "The tenant is archived and hidden from the list. You can restore it from the Archived view.",
      okText: "Delete",
  ...
  function restore(t: Tenant) {
    update.mutate(
      { slug: t.slug, name: t.name, status: "active" },
      { onSuccess: () => message.success("Tenant restored") },
    );
  }
  ...
  const columns: TableColumnsType<Tenant> = [
    {
      ...
      render: (_, t) => (
        <Space>
          <Avatar style={{ backgroundColor: "#2f54eb", flexShrink: 0 }}>
          ...
          <Space>
            ...
            <Tooltip title="Preview">
              <Button
                ...
                aria-label="Preview"
                icon={<EyeOutlined />}
                onClick={() => navigate(`/t/${t.slug}?tab=preview`)}
              />
            </Tooltip>
            <Tooltip title="Version history">
              <Button
                ...
                onClick={() => navigate(`/t/${t.slug}?tab=versions`)}
              />
            </Tooltip>
