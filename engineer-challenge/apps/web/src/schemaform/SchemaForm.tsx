...
export function SchemaForm({ schema, config, onChange, errors }: Props) {
  const errorsFor = (key: string) =>
    errors.filter(e => e.field === key || e.field.startsWith(key + '.') || e.field.startsWith(key + '['))
  return (
      ...
      {schema.dimensions.map(dim => (
        <section key={dim.key} className="rounded-lg border bg-white p-4">
          ...
          <div className="space-y-4">
            {dim.ui.filter(d => isVisible(d, config)).map(d => {
              const Widget = WIDGETS[d.widget] ?? FallbackWidget
              return (
                <Widget key={d.key} descriptor={d} value={getByPath(config, d.key)} config={config}
                  errors={errorsFor(d.key)} onChange={(v: unknown) => onChange(setByPath(config, d.key, v))} />
              )
            })}
          </div>
        </section>
